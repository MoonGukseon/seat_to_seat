class HomeController < ApplicationController
  ...
  def index
    post=Post.select('post_title').order("RANDOM()").limit(15)
  ...
  def p_perfo
    ...
    if "all".eql? region 
      @place=Place.where(:type1 => '공연')
      @perfo=Performance.where(:place_id => @place)  
    else
  ...
  def r_sport
    ...
    if "all".eql? region 
      if type2.nil?
        ...
        @place=Place.where(:type1 => '스포츠', :type2=> params[:type2])
      end
    else
  ...
  def r_theater
    ...
    if "all".eql? region 
      if type2.nil?
        ...
        @place=Place.where(:type1 => '극장', :type2=> params[:type2])
      end
    else
      if !region.nil? && type2.nil?
